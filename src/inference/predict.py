...
class ChatbotPredictor:
    def __init__(self, model_dir: str | Path, faq_path: str | Path, confidence_threshold: float = 0.60) -> None:
    ...
    def predict(self, question: str) -> dict:
        inputs = self.tokenizer(question, return_tensors="tf", truncation=True, padding=True)
        logits = self.model(inputs).logits
        probs = tf.nn.softmax(logits, axis=1).numpy()[0]
        pred_id = int(np.argmax(probs))
        confidence = float(probs[pred_id])
        intent = self.id_to_label[pred_id]
        intent, confidence = apply_fallback(intent, confidence, self.confidence_threshold)
        answer = get_fallback_answer() if intent == "unknown" else get_answer_for_intent(intent, self.faq_df)
        return {"intent": intent, "confidence": confidence, "answer": answer, "model_version": "local"}
